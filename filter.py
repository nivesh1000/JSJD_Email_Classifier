...
def classify_emails(emails: List[Dict], groups: List[Dict]) -> List[Dict]:
    ...
    try:
        ...
        try:
            for group in groups:
                ...
                if not group.get("keywords"):
                    default_group = group
                    logger.debug(f"Default group found: {default_group['id']}")
                    break
        except Exception as e:
            logger.error(f"Error finding default group: {str(e)}")
            default_group = None  # Proceed without default group if error occurs
        ...
        try:
            for group in groups:
                ...
                if group.get("name") == "Unsubscribe Requests":
                    ...
                    logger.debug(f"Unsubscribe group found: {unsubscribe_group['id']}")
                    break
        except Exception as e:
        ...
        for email in emails:
            try:
                    ...
                    try:
                        for keyword_data in unsubscribe_group.get("keywords", []):
                            if not isinstance(keyword_data, dict):
                                logger.warning(f"Invalid keyword data: {keyword_data}")
                                continue
                            keyword = str(keyword_data.get("keyword", "")).lower()
                            keyword_id = keyword_data.get("id")
                            if not keyword or not keyword_id:
                                logger.warning(f"Missing keyword or ID in: {keyword_data}")
                                continue
                            ...
                            if keywordmatcher(subject, keyword) or keywordmatcher(body, keyword):
                                ...
                                logger.info(f"Email classified as unsubscribe: {subject}")
                ...
                for group in groups:
                    try:
                        if not isinstance(group, dict) or "id" not in group:
                            logger.warning(f"Skipping invalid group: {group}")
                            continue
                        group_id = group["id"]
                        ...
                        for keyword_data in group.get("keywords", []):
                            ...
                            if keywordmatcher(subject, keyword) or keywordmatcher(body, keyword):
                                ...
                                if keyword_id not in matched_groups[group_id]:
                                    ...
                                    logger.debug(f"Matched keyword '{keyword}' for group {group_id} in email: {subject}")
                    except Exception as e:
                ...
                try:
                    ...
                    logger.error(f"Error formatting groups for email {subject}: {str(e)}")
                ...
                if not email["group"] and default_group:
                    try:
                        ...
                        logger.info(f"Assigned default group to email: {subject}")
                    except Exception as e:
                        logger.error(f"Error assigning default group to email {subject}: {str(e)}")
                        email["group"] = []  # Reset to empty list on error
        ...
        logger.error(f"Critical error in classify_emails: {str(e)}")
...
def keywordmatcher(text: str, keyword: str) -> bool:
    ...
    try:
        if not isinstance(text, str) or not isinstance(keyword, str):
            logger.warning(f"Invalid input to keywordmatcher - text: {type(text)}, keyword: {type(keyword)}")
            return False
        return bool(re.search(r'\b' + re.escape(keyword) + r'\b', text, re.IGNORECASE))
    except re.error as e:
        logger.error(f"Regex error in keywordmatcher with keyword '{keyword}': {str(e)}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error in keywordmatcher with keyword '{keyword}': {str(e)}")
        return False